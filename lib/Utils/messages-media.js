...
async function getMediaKeys(buffer, mediaType) {
    ...
    const expandedMediaKey = await crypto_1.hkdf(buffer, 112, {
...
const extractVideoThumb = (videoPath, time = '00:00:00', size = {
    width: 320
}) => {
...
const getStream = async (item, opts) => {
    ...
    return {
        stream: fs_1.createReadStream(item.url),
        type: 'file'
    }
}
...
async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        const {
            buffer,
            original
        } = await extractImageThumb(file, 256, 95)
        ...
        try {
            const buff = await extractVideoThumb(file, '00:00:00', {
                width: 32,
                height: 32
            })
...
const getMediaRetryKey = (mediaKey) => {
    return crypto_1.hkdf(mediaKey, 32, {
