...
const makeNewsletterSocket = (config) => {
        ...
        return await Promise.all(WABinary_1.getAllBinaryNodeChildren(child).map(async (messageNode) => {
            ...
            if (type === 'messages') {
                const {
                    fullMessage: message,
                    decrypt
                } = await Utils_1.decryptMessageNode(messageNode, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, config.logger)
                await decrypt()
    ...
    return {
        ...
        newsletterUpdatePicture: async (jid, content) => {
            const {
                img
            } = await Utils_1.generateProfilePicture(content)
            await newsletterWMexQuery(jid, Types_1.QueryIds.JOB_MUTATION, {
                updates: {
